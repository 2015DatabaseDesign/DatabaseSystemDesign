...
Tree::Tree():StrLen(10), N(3), LeafLeast(ceil( (N-1)/2.0)), InnodeLeast(ceil(N/2.0) - 1) {};
...
int Tree::search_leaf(const string& database, const string& table_name, const index_info& inform) {
    blockInfo *root = get_file_block(database, table_name, IndexFile, 0);
    blockInfo *node = get_file_block(database, table_name, IndexFile, root->blockNum);
    if (node == NULL)
        return -3;
    ...
    while (info[0] != '!') {
        ...
        node = get_file_block(database, table_name, IndexFile, blocknum);
        if (node == NULL)
            return -3;
        info = node->cBlock;
    }
...
int Tree::search_one(const string& database, const string& table_name, struct index_info& inform) {
    ...
    if (inform.type == Int)
        ...
        length = StrLen;
    blockInfo *node = get_file_block(database, table_name, IndexFile, blocknum);
    if (node == NULL) {
        inform.offset = 0;
        return -3;
    }
...
int Tree::findNextLeafSibling(const string& database, const string& table_name, int blocknum) {
    blockInfo *node = get_file_block(database, table_name, IndexFile, blocknum);
    if (node == NULL)
        return -3;
    string info = node->cBlock;
...
int Tree::findLeftMostSibling(const string& database, const string& table_name) {
    blockInfo *node = get_file_block(database, table_name, IndexFile, 0);
    if (node == NULL)
        return -3;
    string left;
    string info = node->cBlock;
    while (info[0] == '?') {
        ...
        node = get_file_block(database, table_name, IndexFile, StrToI(left));
        info = node->cBlock;
    }
    return StrToI(left);
}

int Tree::findPrevLeafSibling(const string& database, const string& table_name, int blocknum) {
    int left = findLeftMostSibling(database, table_name);
    if(left == blocknum)
        return -1;
    blockInfo* Left = get_file_block(database, table_name, IndexFile, left);
    if(Left==NULL)
        return -3;
    string info = Left->cBlock;
    while (info.substr(Left->charNum - 3, 3) != IToStr(blocknum, 3)) {
        left = StrToI(info.substr(Left->charNum - 3, 3));
        Left = get_file_block(database, table_name, IndexFile, left);
        if (Left == NULL)
            return -3;
        info = Left->cBlock;
    }
...
int Tree::findParent(const std::string& database, const std::string& table_name, const index_info& inform, int blocknum) {
    blockInfo *node = get_file_block(database, table_name, IndexFile, blocknum);
    if (node == NULL)
        return -3;
    int length, parent;
    ...
    while (root->blockNum != blocknum) {
        ...
        root = get_file_block(database, table_name, IndexFile, blockNum);
        info = root->cBlock;
    }
    return parent;
}

void Tree::search_many(const std::string& database, const std::string& table_name, int& start, int& end, int type, struct index_info& inform) {
    int length, next;
    if (inform.type == Int)
        length = IntLen;
    else if (inform.type == Float)
        ...
        length = StrLen;
    int blocknum = search_leaf(database, table_name, inform);
    blockInfo *leaf = get_file_block(database, table_name, IndexFile, blocknum);
    ...
    if (type == Greater) {
        if (info[info.size()-1] == '#') { // last node
            ...
            if (compare >= 0) {
                ...
                string tmp = get_file_block(database, table_name, IndexFile, start)->cBlock;
        ...
        if (leaf->blockNum == start) {
            ...
            if (compare <= 0) {
                ...
                string tmp = get_file_block(database, table_name, IndexFile, end)->cBlock;
                inform.offset = getoffset(database, table_name, inform, type, end);
            }
            else {
                end = leaf->blockNum;
...
int Tree::getoffset(const std::string& database, const std::string& table_name, const index_info& inform, int type, int blocknum) {
    blockInfo *leaf = get_file_block(database, table_name, IndexFile, blocknum);
    string info = leaf->cBlock;
...
void Tree::delete_one(const string& database, const string& table_name, struct index_info& inform) {
...
void Tree::delete_entry(const string& database, const string& table_name, struct index_info& inform, int n, const string& K, int nod) {
    int length;
    if (inform.type == Int) {
        length = IntLen;
        setN(340);
    ...
    blockInfo *Node = get_file_block(database, table_name, IndexFile, n);
    string info = Node->cBlock;
    string originN = info;
    ...
    for (int i = 0; i < num; i++) {
        int compare;
        if (tmpN[0] == '?')
            start = (length+ChildLen)*i + ValueLen + ChildLen;
        else if (tmpN[0] == '!')
            start = (length+LeafLen)*i + ValueLen + LeafLen;
    ...
    if (Node->blockNum == 0 && originN.substr(1, 4) == "0001") { // root has only one child
        blockInfo *child = get_file_block(database, table_name, IndexFile, StrToI(tmpN.substr(5,3)));
        ...
        if (info[0] == '!') { // leafnode
            if (StrToI(info.substr(1, 4)) < LeafLeast) {
                ...
                blockInfo *N1 = get_file_block(database, table_name, IndexFile, n1);
                ...
                if (parent == parent1) {
                    ...
                    if (StrToI(info.substr(1, 4)) + StrToI(tmpN1.substr(1, 4)) <= N-1) { // merge
						...
						blockInfo* Parent = get_file_block(database, table_name, IndexFile, parent);
...
void Tree::insert_one(const std::string& database, const std::string& table_name, struct index_info& inform) {
    blockInfo *root = get_file_block(database, table_name, IndexFile, 0);
    ...
    blockInfo *node = get_file_block(database, table_name, IndexFile, Node);
    string info = node->cBlock;
    if (StrToI(info.substr(1, ValueLen)) < N-1)
        insert_leaf(database, table_name, inform, Node);
    else {  // divide T - L1
        blockInfo *L1 = get_new_block(database, table_name);
        int l1 = L1->blockNum;
        blockInfo *T = get_new_block(database, table_name);
...
void Tree::insert_leaf(const string& database, const string& table_name, struct index_info& inform, int Node) {
    blockInfo *leaf = get_file_block(database, table_name, IndexFile, Node);
    string info = leaf->cBlock;
    int count = StrToI(info.substr(1, ValueLen));
...
void Tree::insert_parent(const std::string& database, const std::string& table_name, struct index_info& inform, int Node, const string& K1, int N1) {
    blockInfo *root = get_file_block(database, table_name, IndexFile, 0);
    int length;
    if (inform.type == Int)
        length = IntLen;
    ...
    if (root->blockNum == Node) { // Node == 0
        blockInfo *R = get_new_block(database, table_name);
        int r = R->blockNum;
    ...
    blockInfo *N0 = get_file_block(database, table_name, IndexFile, Node);
    int p = findParent(database, table_name, inform, N0->blockNum);
    blockInfo *P = get_file_block(database, table_name, IndexFile, p);
    ...
    if (StrToI(info.substr(1, 4)) < N-1) {
        ...
        blockInfo *P1 = get_new_block(database, table_name);
        int p1 = P1->blockNum;
...
void Tree::write(blockInfo *const node, const string& s) {
    if (node->cBlock)
        delete[] node->cBlock;
    node->cBlock = new char[s.size() + 1];
